...
public class dp_helper extends SQLiteOpenHelper {
    private  static final  String dbname="product_db1";
    SQLiteDatabase moviedb;
    public dp_helper(@Nullable Context context)  {
        super(context, "product_db1", null, 3);
    }
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        ...
        sqLiteDatabase.execSQL("create table orders(ordid integer primary key autoincrement" +
                ",orddate date ,custid integer ,adress text " +
...
//sqLiteDatabase.execSQL("insert into cutomers(custname,username,password,gender,birthdate,job) values("mohamed ahmed","mohamed")");
    ...
    public  void insert_all_products(SQLiteDatabase sqLiteDatabase)
    {
        ...
        row1.put("pro_name","computer");
        row1.put("price","1000");
        row1.put("quantity","100");
        row1.put("catid",3);
        //pass customerid through the forms
        sqLiteDatabase.insert("products",null,row1);
    ...
    public void update_quantities( Integer pro_id,Integer new_price,Integer amount)
    {
        moviedb=getWritableDatabase();
        ContentValues row=new ContentValues();
        row.put("price",new_price);
        row.put("Amount",amount);
        moviedb.update("shopping_card",row,"product_id like ?",new String[]{String.valueOf(pro_id)});
        moviedb.close();
    }
    public void delete_quantities(Integer product_id)
    {
        moviedb=getWritableDatabase();
        moviedb.delete("shopping_card","product_id = "+product_id,null);
        moviedb.close();
    }
